...
class fooocusClient:
    ...
    def list_sizes(self):
        for s in self.sizes:
            print(f"{s} : {self.get_ratio(s)}", end= ', ')
        return self.sizes

    def get_ratio(self, size: str):
        ratio = size.replace("×","/")
        frac = Fraction(ratio).limit_denominator(100)
        return str(frac)

    def lists_styles(self):
        print(self.styles)
        return self.styles
    ...
    def check_performance(self, performance):
        if performance not in self.performances:
